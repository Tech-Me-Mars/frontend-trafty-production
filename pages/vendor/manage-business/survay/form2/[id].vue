...
<template>
  ...
  <div class="bg-zinc-100 min-h-screen">
    <!-- <van-nav-bar :title="t('แบบตรวจสอบด้านความปลอดภัย')" left-arrow
      @click-left="navigateTo(`/vendor/manage-business/survay/form1/${route.params.id}`)">
    </van-nav-bar> -->
<LayoutsBaseHeader :title="t('แบบตรวจสอบด้านความปลอดภัย')" :showBack="true" :back-to="`/vendor/manage-business/survay/form1/${route.params.id}`">
        </LayoutsBaseHeader>
    <div class="p-4 ">
      <Form :validation-schema="yupSchema" @submit="onSubmit">
...
const loadSurvey = async () => {
  try {
    const res = await dataApi.geySurveyBuId(2);
...
const onSubmit = async (values) => {
  ...
  navigateTo(`/vendor/manage-business/survay/form3/${route.params.id}`);
};
